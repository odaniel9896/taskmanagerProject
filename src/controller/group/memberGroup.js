...
module.exports = {
  ...
  async addMemberGroup(req, res) {
    ...
    try {
      ...
      if (!group) return res.status(404).send({ error: "Grupo não existe" });

      if (group) {
        if (userRole == "teacher") await group.addTeacher(userId);

        if (userRole == "student") await group.addStudent(userId);
      } else {
        return res.status(404).send({ error: "Usuário não encontrado" });
      }
