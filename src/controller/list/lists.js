...
module.exports = {
  ...
  async store(req, res) {
    ...
    try {
      ...
      if (!group.Workspace.id)
        return res
          .status(404)
          .send({ error: "Esse grupo não tem uma workspace" });
  ...
  async delete(req, res) {
    ...
    try {
      ...
      if (userRole === "teacher") association = "Teachers";
      else association = "Students";
      ...
      if (list.workspaceId !== group.Workspace.id)
        return res
          .status(404)
          .send({ error: "Essa lista não pertence a esse workspace" });
